...
if (!empty($_POST)) {
	...
	if (empty($_POST['nip']) || empty(md5($_POST['pass'])) || empty($_POST['pass'])) {
...
$pass_biasa = $_POST['pass'];
$pass_row   = $row['pass'];

        if ($pass_md5 == $pass_row) {
            $login_nya_ok = true;
	...
	if ($cek_level_ok_1) {
		...
		$jawaban_nya["level"] = 2;
        	...
        	$jawaban_nya["tampilkan_pesan"] = "Selamat Datang Petugas\nVersi Terbaru Telah Tersedia\nSilahkan Download di Website sppdrssm.rssoedonomadiun.co.id atau\nHubungi Administrator\nExt : 146 [IPDE]";

                $query = "insert into login_activity  (nip, tgl_bln_thn, waktu, versi) values (:nip_login, :tgl_skrng ,:jam_skrng,:versi)";
