<div class="col-md-12">
	...
	<div id="divserahterima" >	
		<div class="card card-outline card-danger">
			...
			<div id="cardermKeperawatanirja_listpasien" style="display: none;">
				<div class="card" >
					<div class="card-header p-2 darkgrey-custom" id="DivermKeperawatanirja_listpasien">
						<div class="row">
							...
							<div class="col-sm-2">  
								...
								<input type="hidden" name="idTransaksiErmKeperawatanIrja" id="idTransaksiErmKeperawatanIrja">
								<input type="hidden" name="idunitErmKeperawatanIrja" id="idunitErmKeperawatanIrja">
							</div>
							<div class="col-sm-2"><button class="btn btn-primary" style="height: 50px;" onclick="kembali()">Kembali</button></div>
						</div>
...
<script type="text/javascript">
	...
	function tampilPasienErmKeperawatanIrja(rm,unit,kunjungan,id_unit,nama,agama,pekerjaan,pendidikan,dpjp,id_transaksi) {
		...
		document.getElementById('ermirja_listpasien2').style.display='none';
		document.getElementById('DivERMKeperawatanRWJ').style.display='none';
		document.getElementById('cardermKeperawatanirja_listpasien').style.display='block';
		tampil_dok_all();
		tampil_ruanganlama(kunjungan);			
		tampil_spesial();
		tampil_dok_awal(unit);
		tampil_rawat_awal(unit);
	}

	function kembali() {
		document.getElementById('ermirja_listpasien2').style.display  ='block';
		document.getElementById('DivERMKeperawatanRWJ').style.display ='block';
		document.getElementById('cardermKeperawatanirja_listpasien').style.display='none';
	}
			...
			if (hasil['data'] !== null) {
				if (hasil['code'] == 'XX') {
					...
					for (var i = 0; i < a.length; i++) {
						...
						Baris += '<i class="fa fa-user"></i>';
						Baris += '</div>';
						Baris += '<a href="#" class="small-box-footer" style="background-color: darkgreen;" onclick="tampilserahterimaermirna('+"'"+norm+"','"+unit+"','"+kunjungan+"','"+id_unit+"','"+nama+"','"+transaksi+"','"+tgl_lahir+"','"+alamat+"'"+')">Klik Detailtttt<i class="fas fa-arrow-circle-right"></i></a>';
						Baris += '</div>';
						Baris += '</div>';
					}
					$('#listserahterimapasien').append(Baris);
  ...
  // document.getElementById('rwipendftr_ruang').innerHTML = ruang;
	...
	function saveSerTerPas(){
		...
		var param = {
			...
			norm 		:$('#rmErmKeperawatanIrja').val(),
			transaksi 	:$('#idTransaksiErmKeperawatanIrja').val(),
			kunjungan 	:$('#idKunjunganErmKeperawatanIrja').val(),
