        ...
        <div class="row row-custom">
              ...
              <div class="input-group input-group-sm mb-3">
                <div class="input-group-prepend">
                  ...
                  <ul class="dropdown-menu">
                    <li class="dropdown-item cri_normKasir" onclick="show_cri_normKasir()">No. RekamMedik</a></li>
                    <li class="dropdown-item cri_nmpasienKasir" onclick="show_cri_nmpasienKasir()">Nama Pasien</a></li>
                  </ul>
                </div>
                <input type="search" class="form-control form-control-xs" placeholder="No. RM..." id="kd_pasiencariKasir" name="kd_pasiencariKasir" autocomplete="off">
                <input type="search" class="form-control form-control-xs" placeholder="Nama Pasien..." id="nm_pasiencariKasir" autocomplete="off">
              </div>
            </div>
          </div>
          <!-- <div class="col-sm-auto">
          ...
          <div class="col-sm-auto">
            <div class="form-group">
              ...
              <select size="1" class="form-control form-control-xs" id="_cari_status_lunas" onchange="caripasien()">
          ...
          <div class="col-sm-auto">
            <div class="form-group">
              ...
              <select size="1" class="form-control form-control-xs" id="_cari_status_tutup_transaksi" onchange="caripasien()">
                <option value="all"> - Semua - </option>
...
<script type="text/javascript">
  ...
  function caripasien() {
    ...
    apiPOST("Kasirgeneral/caripasien", param, hasil => {
      if (hasil['data'] !== null) {
        //alert('tes');
        if (hasil['code'] == 'XX') {
        ...
        for (var i = 0; i < a.length; i++) {
          ...
          Baris += '<tr class="odd" ondblclick="caritransaksikasir(' + a[i].id_transaksi + ');">';
          Baris += '<td>' + no + '</td>';
          Baris += '<td >' + tutup + '</td>';
          Baris += '<td >' + status + '</td>';
        ...
        document.getElementById("listtableKasir").innerHTML = Baris;
        kasirgeneralrefresh();
        $('#loading_kasir').hide();
  ...
  //     var no = i + 1;
  //     Baris += '<td>' + no + '</td>';
  //     Baris += '<td onclick="caripasienKasir()">LUNAS</td>';
  //     Baris += '<td onclick="caripasienKasir()">10000' + no + '</td>';
  //     Baris += '<td onclick="caripasienKasir()">' + no + '-Feb-2023</td>';
  //     Baris += '<td>000' + no + '</td>';
  ...
  function caripasienKasir() {
    $('.dtlKasir').load('Kasirgeneral/mod_kasir/');
  }
  ...
  //   //alert(id);
  //   $('.dtlKasir').load('Kasirgeneral/mod_kasir?data=' + myJSON);
  // })
  ...
  function caritransaksikasir(val_idtransaksi) {
    ...
    apiPOST("Kasirgeneral/cek_kunjungan", json_data, hasil => {

      //alert('tes');
      if (hasil['code'] == 'XX') {
        $('#loading_kasir').hide();
        toastr.error('Silahkan Ulangi Daftar Kunjungan Pasien');
        var user = JSON.parse(localStorage['data_user']);
        var id_user = user['id_user'];
        pertanyaan.fire({
          title: 'Pendaftaran Kunjungan gagal',
          html: '<span>Apakah Nomor Transaksi : ' + idtransaksi + ' akan dihapus,<br>setelah hapus transaksi silahkan hubungi admisi untuk didaftarkan ulang secepatnya</span>',
          icon: 'question',
          ...
          if (result.isConfirmed) {
            ...
            apiPOST('Kasirgeneral/Kasir_deletetransaksinyantol', param, hasil => {
  ...
  function tamp_depositpasien() {
    $('.lookkup').load('Kasirgeneral/mod_deposit');
  }
  ...
  $("#pencariankasir").submit(function(event) {
    caripasien();
  ...
  function awal() {
    ...
    apiPOST('Kasirgeneral/lihatkasuser', param, hasil => {
      var kas_sekarang = hasil['data'];
      document.getElementById("namakas").value = kas_sekarang;
    })
    ...
    apiPOST('Kasirgeneral/lihatshifkasir', null, hasil => {
      ...
      pertanyaan.fire({
        ...
        html: '<span> Sekarang anda masuk shift <b>' + shiftsekarang + ' ' + kas + '</b></span>',
        icon: 'warning',
